...
5. params에 days, strategy_name, forward_days 등 tool 파라미터를 포함하세요.
   - days 기본값: 365 (최근 1년, 대시보드 기본 기간과 동일).
     사용자가 "최근 3개월", "6개월" 등 기간을 언급하면 해당 값 사용.
   - forward_days 기본값: 5.
...
8. **종목/기간/시점 명시**: 응답의 summary 또는 analysis 첫 부분에
   분석 대상 종목명(자산 ID), 데이터 기준일, 분석 기간을 반드시 포함하세요.
   - 데이터에 accuracy_period나 period 정보가 있으면 해당 기간을 사용하세요.
   - 기본 분석 기간: 최근 1년 (365일).
     예: "삼성전자(005930), 2026-03-21 기준, 최근 1년 분석"."""

...
## Phase F: Full Agentic Flow — ✅ 완료 (10/10)
> dev-docs: `dev/active/phaseF-agentic/`
> **파일 집계**: 신규 ~6 / 수정 ~6 / Migration 0

### Stage A: 기반 정의
- [x] F.1 Pydantic 스키마 정의 (ClassificationResult, CuratedReport, UIActionModel) `[S]`
- [x] F.2 Knowledge Expert Prompts (Classifier + 4개 페이지 전문가) `[S]`

### Stage B: 핵심 모듈 구현
- [x] F.3 LLM Classifier (JSON mode, gpt-5-mini) `[M]`
- [x] F.4 DataFetcher (tool 프로그래밍적 호출, 동적 매핑) `[M]`
- [x] F.5 LLM Reporter (JSON mode, knowledge prompt) `[M]`

### Stage C: 백엔드 통합
- [x] F.6 chat_service.py 통합 (agentic flow 전환) `[L]`

### Stage D: 프론트엔드 확장
- [x] F.7 follow_up SSE + 프론트엔드 UI `[S]`
- [x] F.8 프론트엔드 navigate 핸들러 (즉시 이동) `[S]`

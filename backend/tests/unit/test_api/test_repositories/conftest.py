...
def seed_assets(db):
    ...
    assets = [
        AssetMaster(asset_id="KS200", name="KOSPI200", category="index", source_priority=["fdr"], is_active=True),
        AssetMaster(asset_id="005930", name="삼성전자", category="stock", source_priority=["fdr"], is_active=True),
        AssetMaster(asset_id="INACTIVE", name="Inactive", category="test", source_priority=["fdr"], is_active=False),
    ]
    db.add_all(assets)
    db.commit()
    return assets
...
def seed_signals(db, seed_assets):
    ...
    rows = [
        ...
        SignalDaily(asset_id="KS200", date=base + datetime.timedelta(days=1), strategy_id="mom_20", signal=-1, score=0.3),
        SignalDaily(asset_id="KS200", date=base, strategy_id="trend_60", signal=1, score=0.9),
        SignalDaily(asset_id="KS200", date=base + datetime.timedelta(days=1), strategy_id="trend_60", signal=0, score=0.5),
    ]
    db.add_all(rows)
    db.commit()
    return rows
...
def seed_backtest(db, seed_assets):
    ...
    base = datetime.date(2026, 1, 5)
    equity = [
        BacktestEquityCurve(run_id=run_id, date=base + datetime.timedelta(days=i), equity=10000 + i * 100, drawdown=0.0)
        for i in range(3)
    ]
    db.add_all(equity)
